...
from config import FILTER_BATCH_SIZE, FILTER_MAX_WORKERS, MODEL_FILTER, SCORE_THRESHOLD
from utils import get_anthropic_client, get_client_names

logger = logging.getLogger(__name__)


def _build_scoring_prompt(articles: List[Dict], client_names: List[str]) -> str:
    ...
    return f"""You are a senior analyst for a MENA-focused business development consultancy.
...
- MENA sovereign wealth funds or state-owned entities investing capital outside the region; intergovernmental agreements that result in investments
...
=== SCORING GUIDE ===
5 — Directly actionable: names a specific company, target market, and either a concrete capital deployment or a clearly defined problem. A BD team member could cite this in a pitch next week.
4 — Strong signal: clearly fits one of the three categories above with named actors and meaningful detail.
3 — Moderate signal: relevant territory but lacks specificity, or tangentially fits our focus.
...
def _score_batch(batch: List[Dict], client_names: List[str], client) -> List[Dict]:
    """Score one batch of articles via a single Claude call. Returns the batch with scores applied."""
    prompt = _build_scoring_prompt(batch, client_names)
    ...
    score_map = {r["url"]: r for r in results if isinstance(r, dict) and "url" in r}
